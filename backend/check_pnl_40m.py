...
try:
    ...
    if isinstance(res, list):
        ...
        forty_mins_ago_ms = now_ms - (40 * 60 * 1000)
        
        fills = sorted(res, key=lambda x: x['time'])
        # Filter for last 40 minutes
        fills = [f for f in fills if f['time'] >= forty_mins_ago_ms]
        ...
        print(f"--- TRADES IN LAST 40 MINUTES ({len(fills)} fills) ---")
